...
class C_ExportExcel extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->session->userdata('tahunGET') != NULL && $this->session->userdata('bulanGET') != NULL) {
            ...
            $tanggal_akhir_GET      = cal_days_in_month(CAL_GREGORIAN, $this->session->userdata('bulanGET'), $this->session->userdata('tahunGET'));

            // Menggabungkan tanggal, bulan, tahun
            $tanggalGET             = $this->session->userdata('tahunGET') . '-' . $this->session->userdata('bulanGET') . '-' . $tanggal_akhir_GET;

            $data = $this->M_SudahLunas->SudahLunasExcel($this->session->userdata('bulanGET'), $this->session->userdata('tahunGET'), $tanggalGET);
            ...
            // Menampilkan tanggal pada akhir bulan GET
            $tanggal_akhir      = cal_days_in_month(CAL_GREGORIAN, $this->session->userdata('bulan'), $this->session->userdata('tahun'));

            // Menggabungkan tanggal, bulan, tahun
            $tanggal             = $this->session->userdata('tahun') . '-' . $this->session->userdata('bulan') . '-' . $tanggal_akhir;

            $data = $this->M_SudahLunas->SudahLunasExcel($this->session->userdata('bulan'), $this->session->userdata('tahun'), $tanggal);
            ...
            /* Excel Header */
            $sheet->setCellValue('A1', 'PT. Urban Teknologi Nusantara');
            $sheet->setCellValue('A2', 'Laporan Pembayaran Customer' . ' Bulan ' . $this->session->userdata('bulan') . ' Tahun ' . $this->session->userdata('tahun'));
            ...
            $date = date('d-m-y-' . substr((string)microtime(), 1, 8));
            $date = str_replace(".", "", $date);
            $filename = "Laporan Pembayaran Customer " . $this->session->userdata('bulan') . " Tahun " . $this->session->userdata('tahun') . substr((string)microtime(), 1, 8);
            $filePath = __DIR__ . DIRECTORY_SEPARATOR . $filename; //make sure you set the right permissions and change this to the path you want
